defmodule RSMP.Node do
    ...
    if channel_configs != [] do
      spawn(fn ->
        ...
        Enum.each(channel_configs, fn config ->
          RSMP.Channels.start_channel(id, config)
        end)
  ...
  defp trigger_initial_reports(id, services) do
    for {service_module, _data} <- services do
      module_name = service_module.name()

      case RSMP.Registry.lookup_service(id, module_name) do
        [{pid, _}] ->
          statuses = RSMP.Service.get_statuses(pid)

          for {code, values} <- statuses do
            RSMP.Service.report_to_channels(id, code, values)
          end
