defmodule RSMP.Client.Web.ClientLive.Index do
  ...
  def connected_mount(_params, _session, socket) do
    ...
    {:ok, pid} = Client.TLC.start_link([])

    {:ok,
     assign(socket,
       rsmp_client_id: pid,
       id: Client.get_id(pid),
       statuses: Client.get_statuses(pid),
       alarms: alarm_map(Client.get_alarms(pid)),
       alarm_flags: RSMP.Alarm.get_flag_keys()
     )}
  end
  ...
  def change_status(data, socket, delta) do
    ...
    Client.set_status(pid, path, new_value)

    if path == "./env/temperature" do
      if new_value >= 30 do
        Client.raise_alarm(pid, path)
      else
        Client.clear_alarm(pid, path)
      end
    end
  ...
  def handle_event("alarm", %{"path" => path, "value" => flag} = _data, socket) do
    pid = socket.assigns[:rsmp_client_id]
    Client.toggle_alarm_flag(pid, path, RSMP.Alarm.flag_atom_from_string(flag))
    {:noreply, socket}
  end
